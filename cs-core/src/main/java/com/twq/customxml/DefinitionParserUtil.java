...
public class DefinitionParserUtil {
     ...
     * @param parentBean The BeanDefinition of the bean who is the parent of the parsed bean (i.e. the Bean that is the parentNode)
     * @param scope The scope to execute in.  Checked if 'step' to provide Spring EL support in a Spring Batch env
     * @throws Exception
     */
    public static void parseLimitedList(ManagedList<Object> objects, Node node,ParserContext ctx, BeanDefinition parentBean, String scope) throws Exception{
    ...
    public static void parseLimitedList(ManagedList<Object> objects, Node node, ParserContext ctx, BeanDefinition parentBean, String scope, boolean supportCustomTags) throws Exception {
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
            ...
            // Either of these is a just a lookup of an existing bean
                ...
                String nameAttr = elem.getAttribute("bean");
                objects.add(ctx.getRegistry().getBeanDefinition(nameAttr));//node.getTextContent()));
            }
    ...
    @SuppressWarnings("deprecation")
    public static Object getObjectByRef(Element elem, ParserContext ctx) throws Exception {
        String nameAttr = elem.getAttribute("ref");
        return ctx.getRegistry().getBeanDefinition(nameAttr);//node.getTextContent()));
    }
    @SuppressWarnings("deprecation")
    public static Object getObjectByClass(Element elem, ParserContext ctx, BeanDefinition parentBean) throws Exception {
        // There is no quick little util I could find to create a bean
        // on the fly programmatically in Spring and still support all
        // Spring functionality so basically I mimic what Spring actually
        // does but on a smaller scale.  Everything Spring allows is
